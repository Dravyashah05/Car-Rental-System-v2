...
function DriversPage() {
  ...
  const loadDrivers = async () => {
    setIsLoading(true)
    try {
      const data = await apiFetch<Driver>('/api/owners/me')
      setDrivers(data ? [data] : [])
    } catch (err) {
      setStatus({
        type: 'error',
        message: err instanceof Error ? err.message : 'Failed to load owner profile',
      })
    } finally {
      setIsLoading(false)
    }
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        const data = await apiFetch<{ user?: { role?: string } }>('/api/auth/profile')
        if (isMounted) {
